...
interface IERC4626 is IERC20, IERC20Metadata {
     ...
     * - MUST NOT revert.
     */
    function assetAddress() external view returns (address assetTokenAddress);
    ...
    /**
     * @dev Returns the amount of assets that the Vault would exchange for the amount of
     ...
     *   would be burned in a withdraw call in the same transaction. I.e. withdraw should
     *   return the same or fewer shares as previewWithdraw if called in the same transaction.
     ...
     * - MUST revert if all of assets cannot be withdrawn (due to withdrawal limit being
     *   reached, slippage, the owner not having enough shares, etc).
     * Note that some implementations will require pre-requesting to the Vault before a
    ...
    /**
     * @dev Allows an on-chain or off-chain user to simulate the effects of their
     ...
     *   withdrawn in a redeem call in the same transaction. I.e. redeem should return
     *   the same or more assets as previewRedeem if called in the same transaction.
     ...
     * - MUST emit the Withdraw event.
     * - MAY support an additional flow in which the underlying tokens are owned by the
     *   Vault contract before the redeem execution, and are accounted for during redeem.
     * - MUST revert if all of shares cannot be redeemed (due to withdrawal limit being
     *  reached, slippage, the owner not having enough shares, etc).
     * NOTE: some implementations will require pre-requesting to the Vault before a
     * withdrawal may be performed. Those methods should be performed separately.
     */
    function redeem(
        uint256 shares,
